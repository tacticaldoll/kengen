#!/usr/bin/env bash
# Naming-worldview guard (see docs/domain-language.md).
#
...
# The softer cases in docs/domain-language.md stay review-governed.
set -euo pipefail
cd "$(dirname "$0")/.."
...
if grep -rnE "\b(struct|enum|trait|type|mod)[[:space:]]+[A-Za-z0-9_]*(${banned})[A-Za-z0-9_]*" --include='*.rs' crates/; then
    echo "" >&2
    echo "naming-guard: an enforcement/engine/I/O/auth-state word names a kengen type/module — see docs/domain-language.md." >&2
